...
class pretraining_dataset(Dataset):
    ...
    def __init__(self, input_file, max_pred_length):
        ...
        keys = ['input_ids', 'input_mask', 'segment_ids', 'intent_ids', 'masked_lm_ids', 'masked_lm_positions', 
                'next_sentence_labels', 'intent_sentence_labels']
    ...
    def __getitem__(self, index):
        ...
        [input_ids, input_mask, segment_ids, intent_ids, masked_lm_ids, masked_lm_positions, 
        next_sentence_labels, intent_sentence_labels] = [
        ...
        return [input_ids, input_mask, segment_ids, intent_ids, 
                masked_lm_labels, next_sentence_labels, intent_sentence_labels]
...
class BertPretrainingCriterion(torch.nn.Module):
    ...
    def forward(self, prediction_scores, seq_relationship_score, intent_match_score, masked_lm_labels,
                next_sentence_labels, intent_match_labels):
        masked_lm_loss = self.loss_fn(prediction_scores.view(-1, self.vocab_size), masked_lm_labels.view(-1))
        next_sentence_loss = self.loss_fn(seq_relationship_score.view(-1, 2), next_sentence_labels.view(-1))
        intent_match_loss = self.loss_fn(intent_match_score.view(-1, 2), intent_match_labels.view(-1))
        total_loss = masked_lm_loss + next_sentence_loss + intent_match_loss
        return total_loss
        ...
        while True:
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    input_ids, input_mask, segment_ids, intent_ids, masked_lm_labels, next_sentence_labels, intent_sentence_labels = batch
                    prediction_scores, seq_relationship_score, intent_match_score = model(input_ids=input_ids, token_type_ids=segment_ids, intent_ids=intent_ids, attention_mask=input_mask)
                    loss = criterion(prediction_scores, seq_relationship_score, intent_match_score, masked_lm_labels, next_sentence_labels, intent_sentence_labels)
                    if args.n_gpu > 1:
