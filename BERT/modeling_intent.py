...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.intent_embeddings = nn.Embedding(config.intent_size, config.hidden_size)
    ...
    def forward(self, input_ids, token_type_ids, intent_ids):
        ...
        intent_embeddings = self.intent_embeddings(intent_ids)

        embeddings = words_embeddings + position_embeddings + token_type_embeddings + intent_embeddings
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.predictions = BertLMPredictionHead(config, bert_model_embedding_weights)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
        self.intent_match = nn.Linear(config.hidden_size, 2)

    def forward(self, sequence_output, pooled_output):
        prediction_scores = self.predictions(sequence_output)
        seq_relationship_score = self.seq_relationship(pooled_output)
        intent_match_score = self.intent_match(pooled_output)
        return prediction_scores, seq_relationship_score, intent_match_score
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, intent_ids, attention_mask):
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids, intent_ids)
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, intent_ids, attention_mask):
        encoded_layers, pooled_output = self.bert(input_ids, token_type_ids, intent_ids, attention_mask)
        sequence_output = encoded_layers[-1]
        prediction_scores, seq_relationship_score, intent_match_score = self.cls(sequence_output, pooled_output)
