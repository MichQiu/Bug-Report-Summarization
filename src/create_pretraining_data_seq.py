...
def create_instances_from_document(
    ...
    max_num_tokens = max_seq_length - 9
    ...
    while i < len(document):
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                special_intent_token_lst_a = []
                for token in tokens_a:
                    if token in special_intent_tokens.keys():
                        tokens_a_intent_idx = special_intent_tokens[token]
                        special_intent_token_lst_a.append(tokens_a_intent_idx)
                tokens_a_intent_count = Counter(special_intent_token_lst_a)
                most_common_intent_a = tokens_a_intent_count.most_common(1)

                special_intent_token_lst_b = []
                for token in tokens_b:
                    if token in special_intent_tokens.keys():
                        tokens_b_intent_idx = special_intent_tokens[token]
                        special_intent_token_lst_a.append(tokens_b_intent_idx)
                tokens_b_intent_count = Counter(special_intent_token_lst_b)
                most_common_intent_b = tokens_b_intent_count.most_common(1)

                print(most_common_intent_a)
                print(most_common_intent_b)

                tokens_a_intent_idx = special_intent_tokens[most_common_intent_a[0][0]]
                tokens_b_intent_idx = special_intent_tokens[most_common_intent_b[0][0]]
                ...
                tokens = [token for token in tokens if token not in special_intent_tokens.keys()]
                print(tokens)
