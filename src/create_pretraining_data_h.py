    ...
    while i < len(document):
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                intent_tokens = {"[DES]": 6, "[QS]": 7, "[CODE]": 8, "[SOLU]": 9, "[INFO]": 10, "[NON]": 11}
                tokens = []
                segment_ids = []
                intent_ids = []
                tokens.append("[CLS]")
                ...
                for token in tokens_a:
                    if token in intent_tokens.keys():
                        tokens_a_intent_idx = intent_tokens[token]
                        break
                ...
                for token in tokens_a:
                    if token in intent_tokens.keys():
                        ...
                        break
                if rng.random() < 0.05:
                    remaining_intent_ids = list(intent_tokens.values())
                ...
                tokens = [token for token in tokens if token not in intent_tokens.keys()]
