...
def create_training_instances(input_files, tokenizer, max_seq_length,
                              ...
                              max_predictions_per_seq, rng, heuristics):
        ...
        for document_index in range(len(all_documents)):
            instances.extend(
                create_instances_from_document(
                    ...
                    masked_lm_prob, max_predictions_per_seq, vocab_words, rng, heuristics))
    ...
    while i < len(document):
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                tokens_a_intent_idx = heuristics.identify_intent_pr_seq(tokens_a, special_intent_tokens)
                tokens_b_intent_idx = heuristics.identify_intent_pr_seq(tokens_b, special_intent_tokens)
...
def main():
    ...
    parser.add_argument("--h_description", default='', type=str)
    parser.add_argument("--h_solution", default='', type=str)
    parser.add_argument("--h_info", default='', type=str)
    ...
    heuristics_files = namedtuple('heuristics_files', 'h_description h_solution h_info')
    h_files = heuristics_files(args.h_description, args.h_solution, args.h_info)

    heuristics = Heuristics(h_files, lang_embeddings=nlp)
